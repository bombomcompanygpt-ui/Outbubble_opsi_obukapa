...
const TesSimulasi: React.FC = () => {
  const { user, addXP, updateProfile, setFocusModeActive } = useStore();
  ...
  // Hide Bubul assistant when in active test/simulation mode
  useEffect(() => {
    if (selectedPlayLevel !== null) {
  ...
  }, [selectedPlayLevel, setFocusModeActive]);
                ...
                <AnimatePresence mode="wait">
                  {!isQuizFinished ? (
                    ...
                    >
                      ...
                      {/* Question Text */}
                      <div className="bg-slate-50 border border-slate-200 p-5 sm:p-6 rounded-2xl text-left shadow-sm relative overflow-hidden">
                        <span className="text-[11px] font-extrabold text-indigo-600 uppercase tracking-wider block mb-1.5">Pertanyaan Studi Kasus</span>
                        ...
                        {questions[currentQuestionIdx]?.options.map((opt, optionIdx) => {
                          ...
                          return (
                            <button
                              ...
                              className={cn(
                                "w-full p-4 sm:p-4.5 rounded-2xl border-2 text-left text-xs sm:text-sm font-bold flex justify-between items-center transition-all cursor-pointer",
                                !isAnswered 
                                  ? "bg-white border-slate-200 hover:border-indigo-600 hover:bg-indigo-50/50 text-slate-800 active:scale-[0.99]" 
                                  : isCorrectOpt
                                    ? "bg-emerald-50 border-emerald-500 text-emerald-950 shadow-sm"
                                    : isSelectedOpt
                                      ? "bg-rose-50 border-rose-500 text-rose-950"
                                      : "bg-slate-50/80 text-slate-400 border-slate-100 opacity-60"
                              )}
                            >
                      ...
                      {showExplanation && (
                        <motion.div 
                          ...
                          className="p-5 bg-indigo-50/90 border border-indigo-200 rounded-2xl flex gap-3 text-slate-800 mt-2 shadow-sm"
                        >
                          <BubulMascot className="w-10 h-10 shrink-0 drop-shadow mt-0.5" />
                          <div className="flex-1">
                            <span className="bg-indigo-900 text-white text-[9px] font-extrabold uppercase px-2.5 py-0.5 rounded-md tracking-wider inline-block">
                              Penjelasan Logika Bubul
                            </span>
                            ...
                            <button
                              ...
                              className="mt-3 bg-indigo-900 hover:bg-indigo-800 text-white font-extrabold text-xs px-4 py-2 rounded-xl shadow transition-all flex items-center gap-1.5 cursor-pointer active:scale-95"
                            >
                              {currentQuestionIdx < 2 ? "Lanjut Soal" : "Lihat Hasil"} <ChevronRight className="w-3.5 h-3.5" />
                            </button>
                          </div>
                        </motion.div>
                      )}
