...
const Materi: React.FC = () => {
  ...
  const { playSound, completeChallenge, setActiveModuleTopic } = useStore();

  // Reset active module topic on unmount
  useEffect(() => {
    return () => {
      setActiveModuleTopic('');
    };
  }, [setActiveModuleTopic]);

  const handleGoToForum = (moduleId: string) => {
    const categoryMap: Record<string, string> = {
      M01: "Algoritma Media Sosial",
      M02: "Filter Bubble",
      M03: "Echo Chamber",
      M04: "Keterbukaan Perspektif",
      M05: "Verifikasi & Nalar Kritis",
    };
    const cat = categoryMap[moduleId] || "Semua Kategori";
    navigate(`/forum?cat=${encodeURIComponent(cat)}`);
  };
  ...
  const handleModuleOpen = (index: number) => {
    ...
    const modId = MATERI_DATA[index]?.id;
    if (modId) {
      setActiveModuleTopic(modId);
    }
    playSound('pop');
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 md:py-16 px-4 md:px-6 font-sans">
      <div className="text-center space-y-4 md:space-y-6 mb-12 md:mb-20">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="inline-block px-5 py-2 rounded-full bg-indigo-50 border border-indigo-100 text-indigo-700 font-extrabold text-xs uppercase tracking-[0.18em] shadow-sm">
          Academic Knowledge Hub
        </motion.div>
        <h2 className="text-4xl md:text-7xl font-black text-slate-900 tracking-tighter leading-tight">
          Katalog <span className="text-gradient">Literasi Kritis</span>
        </h2>
        <p className="text-slate-600 text-base md:text-xl max-w-2xl mx-auto font-medium">
          Pelajari modul terstruktur untuk memahami algoritma, psikologi media sosial, dan metode verifikasi informasi.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
        {MATERI_DATA.map((module, index) => (
          <motion.div
            ...
            className={cn(
              "group p-6 md:p-10 cursor-pointer rounded-[28px] md:rounded-[40px] border-2 transition-all shadow-md hover:shadow-xl flex items-center gap-4 md:gap-8 bg-white",
              module.borderColor
            )}
          >
            <div className={cn("w-16 h-16 md:w-24 md:h-24 rounded-[20px] md:rounded-[30px] flex items-center justify-center text-white shadow-lg shrink-0", module.color)}>
              <module.icon className="w-8 h-8 md:w-10 md:h-10" strokeWidth={2.5} />
            </div>
            <div className="flex-1 min-w-0">
              <span className="text-[10px] md:text-xs font-black text-indigo-600 uppercase tracking-widest">{module.id} MODUL</span>
              <h3 className="text-xl md:text-2xl font-black text-slate-900 leading-snug mb-1 md:mb-2 mt-1 truncate md:whitespace-normal group-hover:text-indigo-700 transition-colors">{module.title}</h3>
              <p className="text-xs md:text-sm text-slate-600 font-medium line-clamp-2 leading-relaxed">"{module.desc}"</p>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <AnimatePresence>
        {activeModule !== null && (
          <motion.div
            ...
            className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-4 lg:p-8 bg-slate-950/85 backdrop-blur-xl"
          >
            <motion.div
              initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }}
              className="bg-white w-full max-w-7xl h-full md:h-[90vh] md:rounded-[44px] overflow-hidden flex flex-col md:flex-row relative shadow-2xl border border-slate-200/80"
              onClick={(e) => e.stopPropagation()}
            >
              <button 
                onClick={handleModuleClose}
                className="absolute top-4 right-4 z-[120] p-3 bg-rose-600 hover:bg-rose-700 text-white rounded-full transition-all shadow-xl md:top-8 md:right-8 cursor-pointer active:scale-95"
                aria-label="Tutup Modul"
              >
                <X size={22} strokeWidth={2.8} />
              </button>

              <div className={cn("w-full md:w-[320px] lg:w-[380px] p-6 md:p-10 flex flex-col shrink-0 relative border-b md:border-b-0 md:border-r border-slate-200/80", MATERI_DATA[activeModule].lightColor)}>
                <div className="flex flex-col h-full z-10">
                  <div className="mb-6 md:mb-8 shrink-0 flex md:flex-col items-center md:items-start gap-4 text-slate-900">
                    <div className={cn("w-12 h-12 md:w-16 md:h-16 rounded-2xl md:rounded-[24px] flex items-center justify-center text-white shadow-xl", MATERI_DATA[activeModule].color)}>
                      {React.createElement(MATERI_DATA[activeModule].icon, { className: "w-6 h-6 md:w-8 md:h-8" })}
                    </div>
                    <div>
                      <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest block">{MATERI_DATA[activeModule].id}</span>
                      <h3 className="text-lg md:text-2xl font-black leading-tight text-slate-900">{MATERI_DATA[activeModule].title}</h3>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-1 gap-2 md:gap-3 md:flex-grow md:overflow-y-auto pr-1">
                    {sectionConfig.map((s) => (
                      <button
                        ...
                        className={cn(
                          "p-3.5 md:p-4 rounded-2xl flex items-center gap-2.5 md:gap-4 font-black transition-all text-left border-2",
                          activeSection === s.key 
                            ? "bg-white text-slate-900 border-indigo-600 shadow-md"
                            : "bg-white/60 border-transparent text-slate-500 hover:bg-white hover:text-slate-800"
                        )}
                      >
                        <s.icon className={cn("w-4 h-4 md:w-5 md:h-5 shrink-0", activeSection === s.key ? "text-indigo-600" : "text-slate-400")} />
                        <span className="text-xs md:text-sm font-extrabold uppercase tracking-wide truncate">{s.label}</span>
                      </button>
                    ))}

                    {/* DISKUSIKAN DI FORUM BUTTON */}
                    <button
                      onClick={() => handleGoToForum(MATERI_DATA[activeModule].id)}
                      className="p-3.5 md:p-4 rounded-2xl flex items-center gap-2.5 md:gap-4 font-black transition-all text-left border-2 bg-indigo-50 border-indigo-200 text-indigo-700 hover:bg-indigo-100 hover:border-indigo-300 shadow-xs cursor-pointer mt-2"
                    >
                      <MessageSquare className="w-4 h-4 md:w-5 md:h-5 text-indigo-600 shrink-0" />
                      <span className="text-xs md:text-sm font-extrabold uppercase tracking-wide truncate">Diskusikan di Forum</span>
                    </button>
                  </div>
                </div>
                <div className="hidden md:block absolute bottom-0 left-1/2 -translate-x-1/2 w-[60%] h-2 bg-indigo-600 rounded-t-xl" />
              </div>

              <div ref={contentRef} className="flex-1 p-6 md:p-12 lg:p-14 overflow-y-auto bg-slate-50/60 flex flex-col relative text-slate-800 border-t md:border-t-0">
                {/* Visual grid accent */}
                <div className="absolute inset-0 bg-[linear-gradient(to_right,#8080800a_1px,transparent_1px),linear-gradient(to_bottom,#8080800a_1px,transparent_1px)] bg-[size:16px_28px] pointer-events-none opacity-60" />

                <div className="max-w-3xl mx-auto w-full flex-grow relative z-10">
                  <div className="flex items-center gap-3 md:gap-4 mb-6 md:mb-8 pb-4 border-b border-slate-200">
                    <div className={cn("w-2 h-10 md:h-12 rounded-full shadow-sm", MATERI_DATA[activeModule].color)} />
                    <div>
                      <h4 className="text-2xl md:text-3xl font-black tracking-tight uppercase leading-none text-slate-900">
                         {sectionConfig.find(s => s.key === activeSection)?.label}
                      </h4>
                      <p className="text-[11px] md:text-xs text-slate-500 font-extrabold uppercase tracking-wider mt-1.5">Eksplorasi Pembelajaran Terstruktur</p>
                    </div>
                  </div>
                  ...
                  <AnimatePresence mode="wait">
                    <motion.div
                      ...
                      className="text-left pb-24 md:pb-10 space-y-6 text-slate-700 text-sm md:text-base leading-[1.8] [&_p]:leading-[1.8] [&_p]:mb-5 [&_p]:text-slate-700 [&_p]:font-normal [&_strong]:text-slate-950 [&_strong]:font-extrabold [&_h5]:text-base [&_h5]:md:text-lg [&_h5]:font-black [&_h5]:text-slate-900 [&_h5]:mt-8 [&_h5]:mb-3 [&_h5]:border-l-4 [&_h5]:border-indigo-600 [&_h5]:pl-3.5 [&_ul]:list-none [&_ul]:space-y-3 [&_ul]:my-5 [&_li]:relative [&_li]:pl-6 [&_li]:text-sm [&_li]:md:text-base [&_li]:text-slate-700 [&_li]:leading-relaxed [&_li]:before:content-['•'] [&_li]:before:absolute [&_li]:before:left-1 [&_li]:before:text-indigo-600 [&_li]:before:font-black"
                    >
                      ...
                      {/* DISKUSIKAN TOPIK INI BANNER */}
                      <div className="mt-8 p-5 md:p-6 rounded-3xl bg-gradient-to-br from-indigo-900 to-[#031466] text-white shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border border-indigo-700/50">
                        <div className="space-y-1 text-left">
                          ...
                          <h4 className="text-base md:text-lg font-black text-white leading-snug">
                            Tertarik membahas materi {MATERI_DATA[activeModule].title}?
                          </h4>
                          <p className="text-xs text-indigo-200 font-normal">
                            Bandingkan analisismu dengan persepsi teman-teman siswa lain di Forum Diskusi.
                          </p>
                        </div>

                        <button
                          onClick={() => handleGoToForum(MATERI_DATA[activeModule].id)}
                          className="px-5 py-3 bg-emerald-500 hover:bg-emerald-600 text-white font-black rounded-2xl text-xs md:text-sm flex items-center gap-2 shadow-md hover:scale-105 active:scale-95 transition-all shrink-0 cursor-pointer"
                        >
                  ...
                  <div className="fixed md:relative bottom-0 left-0 right-0 p-4 md:p-0 md:mt-12 bg-white md:bg-transparent border-t md:border-t-0 border-slate-200 flex items-center justify-between z-50">
                    <button 
                      onClick={handlePrevModule}
                      disabled={activeModule === 0}
                      className="flex items-center gap-1.5 md:gap-2 text-xs md:text-sm font-extrabold text-slate-500 hover:text-slate-900 disabled:opacity-0 transition-all cursor-pointer"
                    >
                      <ChevronLeft size={18} /> Modul Sebelumnya
                    </button>

                    <button 
                      onClick={handleNextSection}
                      className={cn(
                        "flex items-center gap-2 px-6 py-3.5 md:px-8 md:py-4 rounded-2xl text-xs md:text-sm font-black text-white shadow-lg hover:shadow-xl transition-all cursor-pointer active:scale-95",
                    ...
                    >
                      {activeSection !== "riset" ? (
                        ...
                        <>Modul Selanjutnya <ArrowRight size={16} /></>
                      ) : (
                        <>Selesai Belajar <GraduationCap size={16} /></>
                      )}
                    </button>
                  </div>
                </div>
