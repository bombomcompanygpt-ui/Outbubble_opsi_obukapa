...
import AlgorithmDetective from './detective/AlgorithmDetective';
...
export default function GamesBubul() {
  ...
  const initialMode = searchParams.get('mode') === 'detective' ? 'detective' : 'menu';
  const [gameMode, setGameMode] = useState<'menu' | 'detective' | 'arcade' | 'quiz' | 'leaderboard'>(initialMode);
  ...
  // --- LEADERBOARD & FIRESTORE STATES (Hanya mencatat skor pengguna asli, tanpa bot/seed) ---
  const [rawFirestoreData, setRawFirestoreData] = useState<any[]>([]);
  const [leaderboardFilter, setLeaderboardFilter] = useState<'all' | 'Arcade Refleks' | 'Kuis Literasi Kritis'>('all');
  const [leaderboardData, setLeaderboardData] = useState<{ userId?: string; name: string; score: number; mode: string; avatar: string; isGuest?: boolean }[]>([]);
  ...
  // Helper to load local saved scores (hanya data asli)
  ...
  // Helper to merge and sort leaderboard items from real user entries only
  const mergeLeaderboards = (firestoreList: any[], filterMode: 'all' | 'Arcade Refleks' | 'Kuis Literasi Kritis' = 'all') => {
    const localList = getLocalLeaderboard();
    const map = new Map<string, any>();

    // 1. Add local list (hanya user asli)
    ...
    // 2. Add firestore list (real-time cross-device entries)
  ...
  useEffect(() => {
    ...
    try {
      ...
      unsub = onSnapshot(lbRef, (snapshot) => {
        ...
        setRawFirestoreData(list);
        const merged = mergeLeaderboards(list, leaderboardFilter);
        setLeaderboardData(merged);
      }, () => {
        setLeaderboardData(mergeLeaderboards([], leaderboardFilter));
      });
    } catch (e) {
      setLeaderboardData(mergeLeaderboards([], leaderboardFilter));
    }
  ...
  // Re-filter when filter mode changes
  useEffect(() => {
    setLeaderboardData(mergeLeaderboards(rawFirestoreData, leaderboardFilter));
  }, [leaderboardFilter, rawFirestoreData]);

  // Save High Score helper
  const saveHighScore = async (finalScore: number, modeName: string) => {
    const username = user?.username || `Penjelajah_#${Math.floor(1000 + Math.random() * 9000)}`;
    const avatar = user?.photoUrl || "https://api.dicebear.com/7.x/avataaars/svg?seed=Felix";
    ...
    try {
      const localList = getLocalLeaderboard();
      const updatedLocal = [
        ...localList.filter((item: any) => !( (item.userId === userId || item.name === username) && item.mode === modeName)), 
        newEntry
      ];
    ...
    try {
      const cleanMode = modeName.replace(/\s+/g, '');
      const cleanId = userId.replace(/[^a-zA-Z0-9_-]/g, '_');
      const docId = `score_${cleanId}_${cleanMode}`;
  ...
  if (gameMode === 'detective') {
    return <AlgorithmDetective onBackToMenu={() => setGameMode('menu')} />;
  }

  return (
    <div className="space-y-6 sm:space-y-8 pb-12 font-sans min-h-[calc(100vh-100px)]">
      ...
      {gameMode === 'menu' && (
        <div className="space-y-6 sm:space-y-8">
          ...
          {/* FEATURED FLAGSHIP CARD: ALGORITHM DETECTIVE */}
          <motion.div
            whileHover={{ y: -4 }}
            className="bg-gradient-to-r from-slate-900 via-[#031466] to-indigo-950 rounded-[36px] p-6 sm:p-10 text-white shadow-2xl border-2 border-cyan-400/40 relative overflow-hidden group"
          >
            {/* Ambient Background Lights */}
            <div className="absolute top-0 right-0 w-96 h-96 bg-cyan-500/15 rounded-full blur-3xl pointer-events-none group-hover:bg-cyan-500/25 transition-all" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-center relative z-10">
              ...
              <div className="lg:col-span-8 space-y-4 text-left">
                <div className="flex flex-wrap items-center gap-2.5">
                  <span className="px-3 py-1 rounded-full bg-cyan-400/20 text-cyan-300 border border-cyan-400/40 font-black text-[11px] uppercase tracking-wider flex items-center gap-1.5 shadow-xs">
                    <Sparkles size={13} className="text-cyan-300" />
                    FITUR UTAMA • MINI-GAME INVESTIGASI
                  </span>
                  <span className="px-2.5 py-1 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-400/30 font-extrabold text-[10px] uppercase tracking-wider">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-sm sm:text-base font-bold text-cyan-200 italic">
                ...
                <p className="text-xs sm:text-sm text-slate-300 leading-relaxed font-medium max-w-2xl">
                  Selidiki bagaimana aktivitas digital siswa (likes, watch time, following) membentuk rekomendasi algoritma di media sosial. Temukan pola <em>filter bubble</em>, evaluasi keputusan, ubah feed menjadi beragam, dan catat refleksi ke buku jurnalmu!
                </p>
                ...
                <div className="flex flex-wrap items-center gap-4 text-xs font-bold text-slate-300 pt-1">
                  <span className="flex items-center gap-1.5 bg-white/10 px-3 py-1.5 rounded-xl backdrop-blur-md">
                    <Layers size={14} className="text-cyan-400" /> 3 Kasus Investigasi
                  </span>
                  <span className="flex items-center gap-1.5 bg-white/10 px-3 py-1.5 rounded-xl backdrop-blur-md">
                    <Clock size={14} className="text-amber-400" /> 5–7 Menit
                  </span>
                  <span className="flex items-center gap-1.5 bg-white/10 px-3 py-1.5 rounded-xl backdrop-blur-md">
                    <Award size={14} className="text-emerald-400" /> +150 XP & Badge
                  </span>
                </div>
              </div>
              ...
              <div className="lg:col-span-4 flex flex-col items-center sm:items-end justify-center gap-3">
                <button
                  ...
                  className="w-full sm:w-auto px-8 py-4.5 bg-gradient-to-r from-cyan-400 via-blue-500 to-indigo-600 hover:from-cyan-300 hover:to-indigo-500 text-slate-950 font-black text-sm sm:text-base rounded-2xl shadow-xl hover:shadow-cyan-500/30 flex items-center justify-center gap-3 cursor-pointer transition-all active:scale-95 group-hover:scale-105"
                >
                  <span>Mulai Investigasi Digital</span>
                  <ArrowRight size={20} className="text-slate-950 font-black" />
                </button>
                <span className="text-[11px] text-cyan-200/80 font-semibold text-center sm:text-right">
                  Didampingi oleh Asisten Bubul 🫧
                </span>
              </div>
          ...
          >
            ...
            <div className="pt-6 space-y-3 relative z-10">
              <div className="flex items-center justify-between text-xs font-bold text-slate-500">
                ...
                <span className="text-[#031466] font-black">
                  {leaderboardData.length > 0 ? `${leaderboardData[0].name} (${leaderboardData[0].score} PTS)` : "Belum ada pemain (0 PTS)"}
                </span>
              </div>
              <button
      ...
      {gameMode === 'leaderboard' && (
        <div className="max-w-4xl mx-auto space-y-6">
          <div className="bg-white rounded-[32px] p-6 sm:p-10 border border-slate-200 shadow-xl space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl sm:text-2xl font-black text-slate-900">Papan Peringkat OutBubble</h2>
                  <p className="text-xs font-semibold text-slate-500">Tersinkronisasi real-time antar semua pemain & penjelajah</p>
                </div>
              </div>

              <button
                onClick={() => setGameMode('menu')}
                className="self-start sm:self-auto px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-800 font-extrabold rounded-xl text-xs transition-all cursor-pointer"
              >
                &larr; Kembali ke Arena
              </button>
            </div>
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
              ...
              <button
                onClick={() => setLeaderboardFilter('Arcade Refleks')}
                className={cn(
                  "px-4 py-2 rounded-xl text-xs font-black transition-all whitespace-nowrap cursor-pointer",
                  leaderboardFilter === 'Arcade Refleks'
              ...
              <button
                onClick={() => setLeaderboardFilter('Kuis Literasi Kritis')}
                className={cn(
                  "px-4 py-2 rounded-xl text-xs font-black transition-all whitespace-nowrap cursor-pointer",
                  leaderboardFilter === 'Kuis Literasi Kritis'
                    ? "bg-indigo-600 text-white shadow-md"
                    : "bg-slate-100 text-slate-600 hover:bg-slate-200"
                )}
              >
                🧠 Kuis Literasi Kritis
              </button>
            </div>

            {/* PERSONAL USER STANDING SUMMARY */}
            {(() => {
              const myEntryIndex = leaderboardData.findIndex(
                item => (item.userId && item.userId === user?.id) || item.name.toLowerCase() === (user?.username || '').toLowerCase()
              );
              const myRank = myEntryIndex !== -1 ? myEntryIndex + 1 : '-';
              const myBestScore = myEntryIndex !== -1 ? leaderboardData[myEntryIndex].score : (user?.xp || 0);

              return (
                <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-slate-900 via-indigo-950 to-[#031466] text-white flex flex-col sm:flex-row items-center justify-between gap-4 shadow-lg border border-indigo-500/20">
                  <div className="flex items-center gap-3.5 w-full sm:w-auto">
                    <div className="w-12 h-12 rounded-2xl bg-white/10 border border-white/20 flex items-center justify-center text-xl overflow-hidden shrink-0">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h4 className="font-extrabold text-sm sm:text-base text-white">{user?.username || 'Penjelajah'}</h4>
                        <span className="px-2 py-0.5 rounded-md text-[10px] font-black uppercase bg-indigo-500/40 text-indigo-200 border border-indigo-400/30">
                          {user?.isGuest ? 'Persona Tamu' : 'Member Resmi'}
                        </span>
                      </div>
                      <p className="text-xs text-indigo-200 font-medium mt-0.5">
                        {myRank !== '-' ? `Peringkat kamu di papan skor saat ini: #${myRank}` : 'Mainkan game untuk mencatat skormu di papan peringkat!'}
                      </p>
                    </div>
                  </div>

                  <div className="flex items-center gap-6 self-end sm:self-auto shrink-0 bg-white/10 px-4 py-2 rounded-xl border border-white/10">
                    <div className="text-center">
                      <span className="text-[10px] font-bold text-indigo-200 uppercase block">Posisi</span>
                      <span className="text-base font-black text-amber-300">#{myRank}</span>
                    </div>
                    <div className="h-6 w-px bg-white/20" />
                    <div className="text-center">
                      <span className="text-[10px] font-bold text-indigo-200 uppercase block">Skor Kamu</span>
                      <span className="text-base font-black text-white">{myBestScore} <span className="text-[10px] font-semibold text-indigo-300">PTS</span></span>
                    </div>
                  </div>
                </div>
              );
            ...
            <div className="space-y-2.5">
              {leaderboardData.length === 0 ? (
                <div className="text-center py-12 px-6 bg-slate-50/90 rounded-3xl border-2 border-dashed border-slate-200 space-y-3">
                  <div className="w-14 h-14 mx-auto rounded-2xl bg-amber-100/90 text-amber-600 flex items-center justify-center text-2xl shadow-inner">
                    🏆
                  </div>
                  <h3 className="font-black text-slate-800 text-base">Papan Peringkat Masih Bersih (0 Pemain)</h3>
                  <p className="text-xs text-slate-500 max-w-md mx-auto leading-relaxed font-medium">
                    Belum ada riwayat permainan dari pengguna. Mainkan kuis literasi kritis atau arcade refleks sekarang untuk menjadi pemain pertama di papan skor!
                  </p>
                  <div className="flex flex-wrap justify-center gap-3 pt-3">
                    <button
                      onClick={() => setGameMode('quiz')}
                      className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-black text-xs rounded-xl shadow-md cursor-pointer transition-all active:scale-95"
                    >
                      🧠 Main Kuis Literasi
                    </button>
                    <button
                      onClick={() => setGameMode('arcade')}
                      className="px-4 py-2.5 bg-cyan-600 hover:bg-cyan-700 text-white font-black text-xs rounded-xl shadow-md cursor-pointer transition-all active:scale-95"
                    >
                      🫧 Main Arcade Refleks
                    </button>
                    <button
                      onClick={() => setGameMode('detective')}
                      className="px-4 py-2.5 bg-amber-500 hover:bg-amber-600 text-white font-black text-xs rounded-xl shadow-md cursor-pointer transition-all active:scale-95"
                    >
                      🔍 Algorithm Detective
                    </button>
                  </div>
                ...
                leaderboardData.map((item, idx) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className={cn(
                        "p-3.5 sm:p-4 rounded-2xl border flex items-center justify-between gap-4 transition-all",
                        isMe ? "ring-2 ring-indigo-500 bg-indigo-50/80 border-indigo-200 shadow-sm" :
                        idx === 0 ? "bg-amber-50/70 border-amber-300 shadow-sm" :
                        idx === 1 ? "bg-slate-50 border-slate-200" :
                        idx === 2 ? "bg-orange-50/60 border-orange-200" : "bg-white border-slate-100 hover:border-slate-200"
                      )}
                    >
                      <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                        <div className={cn(
                          ...
                          idx === 2 ? "bg-gradient-to-br from-orange-400 to-orange-500 text-white" : "bg-slate-100 text-slate-600"
                        )}>
                          {idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : `#${idx + 1}`}
                        </div>

                        <div className="flex items-center gap-3 min-w-0">
                          <div className="w-10 h-10 rounded-xl bg-slate-100 border border-slate-200 flex items-center justify-center font-bold text-xs text-slate-800 overflow-hidden shrink-0">
                            {item.avatar && (item.avatar.startsWith('http') || item.avatar.startsWith('/')) ? (
                              <img src={item.avatar} alt="Avatar" className="w-full h-full object-cover" />
                            ) : (
                              <span>{item.avatar || item.name.slice(0, 2).toUpperCase()}</span>
                            )}
                          </div>
                          <div className="min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                              <h4 className="font-extrabold text-slate-900 text-xs sm:text-sm truncate max-w-[160px] sm:max-w-[240px]">{item.name}</h4>
                              {isMe && (
                                <span className="px-1.5 py-0.5 rounded-md text-[9px] font-black uppercase bg-indigo-600 text-white shrink-0">
                                  Kamu
                                </span>
                              )}
                            </div>
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">{item.mode || "Game Pop"}</span>
                          </div>
                        </div>
                      </div>

                      <div className="text-right shrink-0">
                        <span className="text-base sm:text-lg font-black text-slate-900">{item.score}</span>
