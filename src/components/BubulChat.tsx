...
Menurut pandanganmu sendiri, dari skala 1 sampai 10, seberapa netral linimasa media sosialmu saat ini? Yuk kita bahas lebih jauh! 🫧`;
  ...
  const renderMessage = (text: string) => {
    return text.split('\n').map((line, i) => {
      ...
      const formattedLine = trimmedLine.replace(/\*\*(.*?)\*\*/g, '<strong class="text-slate-900 font-extrabold bg-indigo-50/80 px-1 rounded">$1</strong>');
      
      if (isBullet) {
        return (
          <div key={i} className="flex items-start gap-2 ml-1 my-1.5 text-slate-800 text-xs md:text-sm leading-relaxed">
            <span className="text-indigo-600 mt-1 shrink-0 text-xs font-black">•</span>
      ...
      return (
        <p key={i} className="mb-2 text-slate-800 text-xs md:text-sm font-medium leading-relaxed" dangerouslySetInnerHTML={{ __html: formattedLine }} />
      );
    });
  };
  ...
  return (
    <motion.div 
      ...
      className={cn(
        "w-full flex flex-col bg-white/95 backdrop-blur-2xl rounded-[32px] shadow-[0_25px_80px_-15px_rgba(15,23,42,0.18)] border border-slate-200 relative overflow-hidden transition-all duration-300",
        isStandalone 
    ...
    >
      {/* HEADER */}
      <div className="relative pt-10 pb-4 px-6 bg-gradient-to-b from-indigo-50/80 to-transparent flex flex-col items-center border-b border-slate-100">
        <motion.div 
          animate={{ y: [-5, 5, -5] }}
        ...
        <div className="absolute top-4 right-4 flex items-center gap-1.5 z-10">
          <button 
            ...
            className="p-2 bg-slate-50 hover:bg-slate-100 text-slate-400 hover:text-indigo-600 rounded-full transition-all shadow-sm flex items-center justify-center disabled:opacity-40 disabled:hover:bg-slate-50 disabled:hover:text-slate-400 cursor-pointer"
          >
            <Trash2 size={16} />
          </button>
          ...
          {onClose && (
            <button 
              onClick={onClose}
              title="Tutup Chat"
              className="p-2 bg-slate-50 hover:bg-rose-50 text-slate-400 hover:text-rose-600 rounded-full transition-all shadow-sm flex items-center justify-center cursor-pointer"
            >
        ...
        <div className="text-center mt-12">
          <h2 className="text-lg font-black text-slate-900 flex items-center justify-center gap-1.5 tracking-tight">
            Bubul Labirin AI <Sparkles size={15} className="text-amber-500 fill-amber-400 animate-pulse" />
          </h2>
          <div className="flex items-center justify-center gap-2 mt-1">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-ping shrink-0" />
            <span className="text-[10px] text-indigo-700 font-extrabold uppercase tracking-widest">Koneksi Gemini Aktif</span>
            <span className="text-slate-300">|</span>
            <span className="text-[10px] text-slate-600 font-bold">Memori ({chatHistory.length} chat)</span>
          </div>
        </div>
      </div>

      {/* AREA PESAN */}
      <div ref={scrollRef} className="flex-1 p-5 overflow-y-auto space-y-4 custom-scrollbar bg-slate-50/50">
        {chatHistory.map((msg, i) => (
          ...
          >
            {msg.role === 'bubul' ? (
              ...
              <div className="w-7 h-7 rounded-full bg-slate-900 text-white flex items-center justify-center shrink-0 shadow-sm">
                <User size={12} />
              </div>
            )}
            ...
            <div className={cn(
              ...
              msg.role === 'bubul' 
                ? "bg-white text-slate-800 rounded-[22px] rounded-bl-[6px] border border-slate-200/80 leading-relaxed font-normal" 
                : "bg-gradient-to-br from-indigo-600 to-blue-700 text-white rounded-[22px] rounded-br-[6px] font-medium leading-relaxed"
            )}>
        ...
        <AnimatePresence>
          {isTyping && (
            ...
            >
               ...
               <div className="px-4 py-2 bg-white rounded-full border border-indigo-100 flex items-center gap-2 shadow-sm">
                  <RefreshCw size={11} className="animate-spin text-indigo-600" />
                  <span className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider animate-pulse">Bubul sedang menganalisis...</span>
               </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <div className="px-5 py-2.5 bg-slate-50 border-t border-slate-200/80">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1.5 no-scrollbar scroll-smooth">
          {SUGGESTED_TOPICS.map((topic, i) => (
            <button
              ...
              disabled={isTyping}
              className="px-3 py-1.5 bg-white hover:bg-indigo-50 border border-slate-200 hover:border-indigo-400 text-[11px] text-slate-700 hover:text-indigo-700 font-bold rounded-full shadow-sm whitespace-nowrap transition-all duration-200 active:scale-95 disabled:opacity-50 shrink-0 cursor-pointer"
            >
              {topic.label}
            </button>
          ))}
      ...
      {/* INPUT AREA */}
      <div className="p-4 bg-white border-t border-slate-200 rounded-b-[32px]">
        <div className="relative flex items-center bg-slate-50 border border-slate-200 focus-within:border-indigo-600 focus-within:bg-white focus-within:ring-2 focus-within:ring-indigo-100 rounded-full transition-all p-1.5">
          <input 
            type="text" 
            value={input}
            ...
            className="w-full pl-4 pr-12 py-2.5 bg-transparent outline-none text-xs md:text-sm font-medium text-slate-900 placeholder:text-slate-400 disabled:opacity-50"
          />
          <button 
            onClick={handleSend}
            disabled={!input.trim() || isTyping}
            className="absolute right-2 w-9 h-9 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-200 disabled:text-slate-400 text-white rounded-full flex items-center justify-center shadow-md active:scale-95 transition-all shrink-0 cursor-pointer"
          >
